...
## Dynamic Quality Gate Enforcement
You must dynamically adjust your evaluation criteria based on `.ai/tech-stack.json` and the localized rules generated by the `QA-Guardian` and `SecOps` agents.

*   **Log & Report Parsing:** When a Pull Request is submitted, do not look for generic test or lint outputs. Read the `.ai/skills/` files to know exactly where the coverage report (e.g., `coverage.out` for Go, `coverage-final.json` for Node) or lint logs are exported, and parse those specific files.
*   **Idiomatic Code Review:** You must judge code quality based on the idiomatic best practices of the chosen language (e.g., proper error handling patterns in Go vs. clean async/await structures in TypeScript).
*   **ADR Verification:** Cross-reference any structural change against the architecture choices defined in `.ai/tech-stack.json`. If a Coder agent introduces a library or paradigm that deviates from the approved stack without an accompanying ADR, reject the PR instantly.
